/*****************************************************************************************************************************************************
 * Objetivo: Arquivo responsavel pela manipulação de dados entre o APP e a Model para o CRUD de filme e ator
 * Data: 12/11/2025
...
// Retorna todos os filmes e ator
const listarFilmesAtor = async function () {
    ...
    try {
        // Chama a função do DAO que retorna a lista de filmes atores 
        ...
        if (result) {
            if (result.length > 0) {
                ...
                MESSAGE.HEADER.response.filmes_ator = result
...
// Retorna um filme e ator filtrando pelo ID
...
// Retorna um filme e ator filtrando pelo ID de filme
const buscarFilmeAtorIdFilme = async function (idFilme) {
    ...
    try {
        ...
        if (idFilme != '' && idFilme != null && idFilme != undefined && !isNaN(idFilme) && idFilme > 0) {
            ...
            let result = await filmeAtorDAO.getSelectByIdFilmFilmActor(parseInt(idFilme))
        ...
        if (String(contentType).toUpperCase() == 'APPLICATION/JSON') {
            ...
            if (!validarDados) {
                ...
                if (result) {
                    ...
                    let lastIdFilmeAtor = await filmeAtorDAO.getSelectLastIdFilmeActor()

                    if (lastIdFilmeAtor) {
                        // Adiciona no JSON de FilmeAtor o ID que foi gerado pelo BD 
...
// Validação dos dados de cadastro ou atualização do filmeAtor
